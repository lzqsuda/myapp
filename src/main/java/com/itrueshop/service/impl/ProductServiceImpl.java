...
public class ProductServiceImpl extends BaseServiceImpl<Product, String>
	...
	public List<Product> getProductList(ProductCategory productCategory,
		...
		return productDao.getProductList(productCategory, firstResult,
				maxResults);
	}
	...
	public List<Product> getProductList(Date beginDate, Date endDate,
			int firstResult, int maxResults) {
		return productDao.getProductList(beginDate, endDate, firstResult,
				maxResults);
	}
	...
	public Pager search(Pager pager) {
		...
		CompassBooleanQueryBuilder compassBooleanQueryBuilder = compassQueryBuilder
				.bool();

		CompassQuery compassQuery = compassBooleanQueryBuilder
				.addMust(compassQueryBuilder.term("isMarketable", true))
				.addMust(
						compassQueryBuilder.queryString(
								"name:*" + pager.getKeyword() + "*").toQuery())
				.toQuery();
		
		if (StringUtils.isEmpty(pager.getOrderBy())
				|| pager.getOrderType() == null) {
			compassQuery
					.addSort("isBest", SortPropertyType.STRING,
							SortDirection.REVERSE)
					.addSort("isNew", SortPropertyType.STRING,
							SortDirection.REVERSE)
					.addSort("isHot", SortPropertyType.STRING,
							SortDirection.REVERSE);
		} else {
	...
	public void delete(Product product) {
		File htmlFile = new File(ServletActionContext.getServletContext()
				.getRealPath(product.getHtmlFilePath()));
		if (htmlFile.exists()) {
			htmlFile.delete();
		}
