...
<body class="list">
	<div class="body">
		<div class="listBar">
			<h1><span class="icon">&nbsp;</span>角色管理&nbsp;<span class="pageInfo">总记录数: ${pager.totalCount}(共${pager.pageCount}页)</span></h1>
		</div>
		<form id="listForm" action="role!list.action" method="post">
			...
			<table class="listTable">
				<tr>
					...
					<th>
						<span class="sort" name="name">角色名称</span>
					</th>
					<th>
				...
				<#list pager.list as list>
					<tr>
						<td>
							<input type="checkbox" <#if list.isSystem>disabled title="系统内置权限不允许删除!"<#else>name="ids" value="${list.id}"</#if> />
						</td>
						<td>
							${list.name}
						</td>
						...
						<td>
							<span title="${list.createDate?string("yyyy-MM-dd HH:mm:ss")}">
								${list.createDate}
							</span>
						</td>
						<td>
							<a href="role!edit.action?id=${list.id}" title="编辑">[编辑]</a>
						</td>
					</tr>
				</#list>
			</table>
