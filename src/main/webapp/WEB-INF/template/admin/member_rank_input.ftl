		...
		<form id="inputForm" class="validate" action="<#if isAdd??>member_rank!save.action<#else>member_rank!update.action</#if>" method="post">
			<table class="inputTable">
				...
				<tr>
					...
					<td>
						<label>
							<input type="radio" name="memberRank.isDefault" value="true"<#if (memberRank.isDefault)!> checked</#if> />
							是
						</label>
						&nbsp;&nbsp;
						<label>
							<input type="radio" name="memberRank.isDefault" value="false"<#if (isAdd || !memberRank.isDefault)!> checked</#if> />
							否
						</label>
					</td>
				</tr>
